...
## Быстрый старт: скачивание и запуск

### Linux
1. В Releases на GitHub скачайте `pimino-<version>-linux-x64.tar.gz` или `pimino-<version>-linux-x64.zip`.
2. Распакуйте:  
   `tar -xzf pimino-<version>-linux-x64.tar.gz`  
   или `unzip pimino-<version>-linux-x64.zip`
3. Перейдите в папку `staging` (если есть) либо в корень распаковки и запустите:  
   `./pimino`
4. При необходимости сделайте файл исполняемым: `chmod +x pimino`.

### Windows
1. В Releases скачайте `pimino-<version>-setup.exe` и установите, либо `pimino-<version>-windows-x64.zip` и распакуйте.
2. Запустите `pimino.exe` из установленной директории или из распакованной папки.
...
- **Как подключить в приложении:**
  ...
  2) Меню «Файл» → «Загрузить SoundFont».  
  3) Выберите ваш `.sf2`.  
  После загрузки звук клавиш и MIDI будет работать.
...
# Команда проекта
* Ганьшин Владислав:
Крутой программист: нашёл библиотеку, применил, собрал проект, внедрил vcpkg, fluidsynth, написал core
* Гордиенко Анастасия:
Идейный вдохновитель, подрядчик, помощник: спланировала проект (техническое задание, диаграмма классов, внешний вид, консультирование по музыке и фортепиано). Разрешила проблему SEGFAULT,
разработала интерфейс для midi файлов, реорганизовала UI, работа с Qt QML
...
## Технические требования к проекту #1
  Соблюдение принципов SOLID, Single Responsibility Princible, DRY, Separation of concerns, Model-View-Delegate, QML
